...
interface PluginSettings {
	apiKey: string;
	useCustomHostInference: boolean;
...
* Self hosting the image scanner may require technical knowledge of docker and requires background processing resources. For most people, the cloud options is better.`;
		...
		new Setting(containerEl).setName("API key").addText((tc) =>
			tc.setValue(this.plugin.settings.apiKey).onChange(async (val) => {
		...
		new Setting(containerEl).setName("Self hosted").addToggle((toggle) =>
			toggle
				.setValue(this.plugin.settings.useCustomHostInference)
				.onChange(async (val) => {
					this.plugin.settings.useCustomHostInference = val;
					await this.plugin.saveSettings();
				})
		);

		new Setting(containerEl).setName("Self Hosted URL").addText((tc) =>
			tc
				.setValue(this.plugin.settings.customHostURL)
				.onChange(async (val) => {
					this.plugin.settings.customHostURL = val;
					await this.plugin.saveSettings();
				})
		);
	}
...
class MathLiveModal extends Modal {
	...
	parseSelection(selectionText: string): {
		...
		if (mathInlineStartIndex >= 0) {
			const mathInlineEndIndex = selectionText.indexOf(
				"$",
			...
			for (const item of clipboardItems) {
				for (const type of item.types) {
					if (item.types.includes("image/png")) {
						...
						this.renderedResult = this.resultRenderTemplate(
							this.mfe?.value ?? ""
						);
	...
	async scanImage(imageData: Blob) {
		...
		if (this.plugin.settings.useCustomHostInference) {
			address = this.plugin.settings.customHostURL;
		}
